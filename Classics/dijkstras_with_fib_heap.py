...
import time
import sys
sys.path.insert(0, "./fibonacci-heap-mod-0.99")
...
def dijkstras_with_fib(cities, num_cities, src_index, dest_index=None):
    """
    ...
    unvisited_heap = fib.FibonacciHeap()
    ...
    while unvisited:
        ...
        for i in unvisited:          # TODO: in general case, use unvisited neighbors (intersect sets)
            ...
            if total_distance < results_list[i].dist:
                ...
                if i not in unvisited_node_dict:
                    ...
                    unvisited_node_dict[i] = unvisited_heap.enqueue(i, total_distance)
                else:
                    unvisited_heap.decrease_key(unvisited_node_dict[i], total_distance)
        ...
        entry = unvisited_heap.dequeue_min()
        min_dist_index = entry.m_elem
...
for _ in range(10):
    start = time.clock()
    results_list = dijkstras_with_fib(cities, num_cities, src)
    end = time.clock()
